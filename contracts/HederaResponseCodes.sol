...
library HederaResponseCodes {
    ...
    int32 internal constant KEY_REQUIRED = 26; //Key not provided in the transaction
    ...
    int32 internal constant KEY_PREFIX_MISMATCH = 68; // one public key matches more than one prefixes on the signature map
    ...
    int32 internal constant EXPIRATION_REDUCTION_NOT_ALLOWED = 109; // The expiration date/time on a smart contract may not be reduced
    int32 internal constant MAX_GAS_LIMIT_EXCEEDED = 110; //Gas exceeded currently allowable gas limit per transaction
    int32 internal constant MAX_FILE_SIZE_EXCEEDED = 111; // File size exceeded the currently allowable limit
    ...
    int32 internal constant TOKEN_SYMBOL_TOO_LONG = 175; // The provided token symbol is too long
